        ...
        const result = cached(`revenue:${req.adminAccount.id}`, () => {
            ...
            for (let i = 0; i < transactions.length; i++) {
                ...
                if (!byDay[day]) {
                    byDay[day] = {
                        date: day,
                        recharge: { diamonds: 0 }, withdraw: { diamonds: 0 },
                        diamondSales: { diamonds: 0 }, diamondCommission: { beans: 0 },
                        other: { diamonds: 0, beans: 0 }
                    };
                }
                const bucket = byDay[day];
                const cat = categorize(t.note);
                const amt = Math.abs(Number(t.amount) || 0);
                if (cat === "recharge") bucket.recharge.diamonds += amt;
                else if (cat === "withdraw") bucket.withdraw.diamonds += amt;
                else if (cat === "diamondSales") bucket.diamondSales.diamonds += amt;
                else if (cat === "diamondCommission") bucket.diamondCommission.beans += amt;
                else bucket.other[t.currency === "diamonds" ? "diamonds" : "beans"] += amt;
            }
