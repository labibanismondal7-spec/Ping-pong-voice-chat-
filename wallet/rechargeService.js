// ==================================================
// RECHARGE / PAYMENT -> DIAMOND SYSTEM (2026-08-16)
// ==================================================
...
//
// Security properties:
//   - diamonds are only ever credited from approveTransaction() below — no
//     other code path in this module touches user.diamonds.
...
//     same record can never credit diamonds twice. The PENDING check and the
...
//   - Every credit is written to an append-only diamond ledger with
...
//   - Package price/diamonds/bonus and all payment settings (UPI ID, enabled
...
function initRechargeService({ DATA_FOLDER, safeRead, safeWrite, findUserByUserId, saveUsers, logTransaction, pushWalletUpdate, clampDiamondBalance } = {}) {
    ...
    const _clampDiamondBalance = typeof clampDiamondBalance === "function"
        ? clampDiamondBalance
        : (userId, diamonds) => (Number.isFinite(diamonds) && diamonds >= 0 ? Math.floor(diamonds) : 0);
...
// Payment destination is intentionally NOT hard-coded. Production must be
// explicitly configured by an authorized admin before recharge is enabled.
if (typeof settings.upiId !== "string") settings.upiId = "";
if (!settings.receiverName) settings.receiverName = "PingPong";
if (!settings.upiId) settings.enabled = false;
let packages = safeRead(PACKAGES_FILE, []);
let packageMigrationNeeded = false;
packages = packages.map((p) => {
    if (p && p.diamonds == null && p.coins != null) { p.diamonds = Math.floor(Number(p.coins) || 0); packageMigrationNeeded = true; }
    if (p && p.bonusDiamonds == null && p.bonusCoins != null) { p.bonusDiamonds = Math.max(0, Math.floor(Number(p.bonusCoins) || 0)); packageMigrationNeeded = true; }
    if (p) { delete p.coins; delete p.bonusCoins; delete p.totalCoins; }
    return p;
});
if (packageMigrationNeeded) safeWrite(PACKAGES_FILE, packages, { immediate: true });
    ...
    function listPackagesPublic() {
        return packages
            ...
            .map((p) => ({
                ...
                totalDiamonds: p.diamonds + (p.bonusDiamonds || 0),
                label: p.label || null
            }));
    }
    ...
    function createPackage(body, adminId) {
        ...
        const diamonds = Number(body && body.diamonds);
        const bonusDiamonds = Number(body && body.bonusDiamonds) || 0;
        if (!Number.isFinite(priceINR) || priceINR <= 0) return { error: "invalid-price", message: "Provide a valid price (INR)" };
        if (!Number.isFinite(diamonds) || diamonds <= 0) return { error: "invalid-diamonds", message: "Provide a valid Diamond amount" };
        const pkg = {
            id: genId("pkg"),
            priceINR,
            diamonds: Math.floor(diamonds),
            bonusDiamonds: Math.max(0, Math.floor(bonusDiamonds)),
    ...
    function updatePackage(id, patch, adminId) {
        ...
        if (patch.diamonds !== undefined) {
            const v = Number(patch.diamonds);
            if (!Number.isFinite(v) || v <= 0) return { error: "invalid-diamonds", message: "Provide a valid Diamond amount" };
            pkg.diamonds = Math.floor(v);
        }
        if (patch.bonusDiamonds !== undefined) {
            const v = Number(patch.bonusDiamonds);
            pkg.bonusDiamonds = Number.isFinite(v) && v >= 0 ? Math.floor(v) : 0;
        }
    ...
    // Never credits diamonds itself — always lands as PENDING. The client
    ...
    function createTransaction({ mobile, userId, packageId, method, utr }) {
        ...
        const txn = {
            id: genId("rcg"),
            userId, mobile,
            packageId: pkg.id, priceINR: pkg.priceINR, diamonds: pkg.diamonds, bonusDiamonds: pkg.bonusDiamonds || 0, totalDiamonds,
            method, utr: cleanUtr,
            status: "PENDING",
            createdAt: new Date().toISOString(),
    ...
    //   approveTransaction()  -> PAYMENT_SUBMITTED -> PAID, diamonds credited
    ...
    //
    // diamonds are still ONLY ever credited inside approveTransaction() — this
    // section adds zero new diamond-crediting code paths.
    ...
    function buildOrderUpiLink(txn, method) {
        ...
        const params = new URLSearchParams({
            ...
            cu: "INR",
            tn: `PingPong Recharge - ${txn.totalDiamonds} diamonds`,
    ...
    async function getOrderPaymentData(id, mobile) {
        ...
        return {
            ...
            totalDiamonds: txn.totalDiamonds,
            method: txn.method,
            status: txn.status
        };
    }
    ...
    // credits diamonds itself — it only moves PENDING -> PAYMENT_SUBMITTED so
    // the order shows up in the admin approval queue. diamonds still only
    ...
    function approveTransaction(id, adminId) {
        ...
        const balanceBefore = found.user.diamonds || 0;
        const balanceAfter = _clampDiamondBalance(txn.userId, balanceBefore + txn.totalDiamonds, "recharge-approve");
        ...
        const ledgerEntry = {
            ...
            txnId: txn.id, amountDiamonds: txn.totalDiamonds,
        ...
        _logTransaction(txn.userId, "diamonds", txn.totalDiamonds, `Recharge approved (${txn.id}, ₹${txn.priceINR} via ${txn.method}, UTR ${txn.utr})`);
