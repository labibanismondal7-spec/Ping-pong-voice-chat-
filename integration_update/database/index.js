...
async function run({ databaseUrl } = {}) {
    ...
    if (!DATABASE_URL) {
        ...
        console.log("[database] DATABASE_URL not set — development startup will use local persistence.");
    ...
    const ran = [];
    const lockKey = 18472931;
    try {
        await pool.query("SELECT pg_advisory_lock($1)", [lockKey]);
        // The base schema must exist before feature migrations: it owns the
        // users/rooms/admin tables and the durable JSON mirror contract.
        const baseSchema = path.join(PACKAGE_ROOT, "..", "db", "schema.sql");
        const walletSchema = path.join(PACKAGE_ROOT, "module4_wallet_ledger", "wallet", "schema.sql");
        const bootstrap = [
            { name: "db/schema.sql", fullPath: baseSchema },
        ...
        for (const item of bootstrap) {
            ...
            for (const statement of sql.split(/;\s*(?=CREATE|ALTER|INSERT|UPDATE|DELETE|DROP|COMMENT|GRANT|REVOKE|DO\b)/i).map(s => s.trim()).filter(Boolean)) { try { await pool.query(statement); } catch (err) { if (!/already exists|duplicate object/i.test(err.message)) throw err; console.warn(`[database] existing object skipped: ${err.message}`); } }
            ran.push(item.name);
        }
        ...
        console.log(`[database] ready — ${ran.length} schema/migration file(s) applied.`);
        return { ran, skipped: false };
    } finally {
        try { await pool.query("SELECT pg_advisory_unlock($1)", [lockKey]); } catch (_) {}
        await pool.end();
    }
}
