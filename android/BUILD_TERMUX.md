...
**Fix (already applied):** `build-termux.sh` (in this same `android/`
folder) auto-detects Termux at run time and passes
`-Pandroid.aapt2FromMavenOverride=...` pointing at Termux's own native
`aapt2`, instead of AGP's incompatible Maven-downloaded one — just run
`./build-termux.sh` instead of `./gradlew` directly (see step 3).

This override is intentionally **not** hardcoded in `gradle.properties`
anymore (2026-08-25 production-readiness fix) — a hardcoded Termux-only
path there broke every other build environment (GitHub Actions, a normal
Linux/Mac/Windows machine) that doesn't have `/data/data/com.termux`. If
your Termux prefix differs from the default, override on the command line:
