...
class MainActivity : AppCompatActivity() {
    ...
    private val mediaPermissionLauncher = registerForActivityResult(
        ...
        // Grant only the WebView resources that map to Android permissions we
        // explicitly support. Android's PermissionRequest documentation warns
        // against blindly granting every resource returned by WebView.
        ...
        if (grantable.size != request.resources.count { it == PermissionRequest.RESOURCE_AUDIO_CAPTURE || it == PermissionRequest.RESOURCE_VIDEO_CAPTURE }) {
            ...
            if (denied.isNotEmpty()) {
                Toast.makeText(
                    this,
                    "Camera/microphone permission is required for calls and video chat.",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
