...
app.post("/api/auth/login-user-id", authLimiter, (req, res) => {
    try {
...
function performTakeSeat(room, { userId, socketId, seatNumber }) {
    ...
    room.seats[seatNumber - 1] = { userId: found.user.userId, socketId, userName: found.user.name, userPhoto: found.user.photo || "", activeFrame: found.user.activeFrame || null, vipLevel: found.user.vipLevel || 0, level: found.user.level || found.user.idLevel || 1, customTag: found.user.customTag || null, agencyBadge: found.user.agencyBadge || null, nameEffect: found.user.nameEffect || null };
    io.to(room.roomId).emit("seat-update", {
...
app.post("/api/messages/send-image", userAuth.requireUserAuth, uploadPhoto.single("photo"), (req, res) => {
...
app.post("/api/admin/users/:mobile/diamonds", requireAdmin, requirePermission("users:diamond-edit"), (req, res) => {
    const u = users[resolveUserKey(req)];
    if (!u) return res.json({ success: false, message: "Not found" });
    if (!actorCanAccessCountry(req.adminAccount, u.countryId)) return countryDeniedResponse(res);
...
function fwResolveSpin(roomId) {
    ...
    const payoutFactor = grossTotal > 0 ? Math.min(FW_MAX_PAYOUT_FACTOR, fundedPool / grossTotal) : 0;
    ...
    Object.keys(g.bets).forEach((userId) => {
        ...
        if (amount > 0 && found) {
            ...
            const balanceAfter = found.user.diamonds;
            // LEVEL SYSTEM UPGRADE 2026-08-04: removed — user.level no longer auto-recomputed from raw coin balance on every coin change (that was the "level increases automatically" bug). Level now only changes via idLevel.js recordGiftSent() (room gift send only), which also mirrors onto this same field. See idLevel.js.
...
io.on("connection", (socket) => {
    ...
    socket.on("send-video-gift", ({ roomId, targetUserId, targetUserIds, videoGiftId, quantity, requestId }) => {
        ...
        try {
            senderFound.user.diamonds = clampDiamondBalance(senderFound.user.userId, (senderFound.user.diamonds || 0) - totalCost, "video-gift-send");
            // LEVEL SYSTEM UPGRADE 2026-08-04: removed — user.level no longer auto-recomputed from raw coin balance on every coin change (that was the "level increases automatically" bug). Level now only changes via idLevel.js recordGiftSent() (room gift send only), which also mirrors onto this same field. See idLevel.js.
            const label = qty > 1 ? `${gift.name} x${qty}` : gift.name;
            ...
            targetList.forEach((targetFound) => {
                ...
                if (receiver) {
                    receiver.user.beans = clampBeansBalance(receiver.user.userId, (receiver.user.beans || 0) + (gift.price * qty), "video-gift-receive");
    ...
    socket.on("mod-move-seat", ({ roomId, targetUserId, seatNumber }) => {
        ...
        io.to(roomId).emit("seat-update", { action: "move", fromSeatNumber: fromIdx + 1, seatNumber: destIdx + 1, userId: targetUserId, socketId: seatData.socketId, userName: seatData.userName, userPhoto: seatData.userPhoto, activeFrame: seatData.activeFrame || null, vipLevel: seatData.vipLevel || 0, level: seatData.level || 1, customTag: seatData.customTag || null, agencyBadge: seatData.agencyBadge || null, nameEffect: seatData.nameEffect || null, activeBadges: seatData.activeBadges || [], role: roleForUser(room, targetUserId) });
        io.to(roomId).emit("room-state", publicRoom(room));
    ...
    socket.on("game-wheel-sync", ({ roomId, balance, game }) => {
        ...
        found.user.diamonds = clampDiamondBalance(socket.userId, before + delta, "game-wheel-sync");
        // LEVEL SYSTEM UPGRADE 2026-08-04: removed — user.level no longer auto-recomputed from raw coin balance on every coin change (that was the "level increases automatically" bug). Level now only changes via idLevel.js recordGiftSent() (room gift send only), which also mirrors onto this same field. See idLevel.js.
