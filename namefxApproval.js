...
// PROFILE_NAME_EFFECT_STYLES to a specific user; Country Manager reviews;
...
// the single Premium Gradient style is the preview — the admin panel can render the same
...
function initNameEffectsApproval(deps) {
    ...
    const namefx = createApprovalWorkflow({
        ...
        validateSubmit(body) {
            ...
            if (!VIP_NAME_EFFECT_STYLES.includes(body.style)) return { ok: false, message: "Select the Premium Gradient style" };
            return {
                ok: true,
