...
function connectSocket() {
  ...
  socket.on("seat-update", (data) => {
    if (data.action === "take") {
      ...
      seatMap[data.seatNumber] = { userId: data.userId, socketId: data.socketId, userName: data.userName, userPhoto: data.userPhoto, role: data.role, activeFrame: data.activeFrame || null, vipLevel: data.vipLevel || 0, level: Number(data.level) || 1, customTag: data.customTag || null, agencyBadge: data.agencyBadge || null, nameEffect: data.nameEffect || null };
      ...
      seatMap[to] = { ...(entry || {}), userId: data.userId, socketId: data.socketId || entry?.socketId, userName: data.userName || entry?.userName, userPhoto: data.userPhoto || entry?.userPhoto, role: data.role || entry?.role, activeFrame: data.activeFrame || entry?.activeFrame || null, vipLevel: data.vipLevel || entry?.vipLevel || 0, customTag: data.customTag || entry?.customTag || null, agencyBadge: data.agencyBadge || entry?.agencyBadge || null, nameEffect: data.nameEffect || entry?.nameEffect || null };
      if (data.userId === me.userId) {
  ...
  socket.on("video-gift-play", (data) => {
    if (data.fromUserId === me.userId && data.gift.price) {
      me.diamonds = Math.max(0, (me.diamonds || 0) - data.gift.price);
  ...
  socket.on("chest-opened", (data) => {
    ...
    if (amRecipient) {
      if (data.reward.type === "diamonds") me.diamonds += data.reward.amount;
  ...
  seats.forEach((seat, i) => {
    ...
    if (seat) {
      ...
      if (seat.micMuted) {
        ...
        muteBadge.src = "/images/icons/icon-mic-muted.png";
        muteBadge.alt = "Muted";
...
function appendGiftMsg(data) {
  ...
  div.innerHTML = `
    <div class="activity-pill gift-pill">
      <span class="who">${escapeHtml(data.fromName || "")}</span>
      <span class="gift-notice-arrow">→</span>
      <span class="who">${escapeHtml(data.toName || "")}</span>
...
function sendGift(giftId) {
  ...
  // real balance regardless; this never deducts Diamonds itself. Opens the
  // existing Wallet/Recharge flow, same as the footer Recharge button.
  if (!me.diamonds) { toast("Not enough Diamonds, check your Wallet"); $("modal-gift").classList.add("hidden"); openWallet(); return; }
...
async function performSettingsLogout() {
  ...
  try {
    if (tokenAtLogout) {
      await fetch(API + "/api/auth/logout", { method: "POST", headers: { "Authorization": "Bearer " + tokenAtLogout } }).catch(() => {});
    }
  } finally {
    ...
    ["pp_user", "pp_auth_token", "pp_room", "pp_room_ts"].forEach(k => localStorage.removeItem(k));
...
$("help-customer-service")?.addEventListener("click", () => { joinRoom("101"); });
...
function showChestReward(data) {
  ...
  // below (animateCountUp) now writes innerHTML instead so the real Diamond/
  // diamond icon image can render inline, same as everywhere else.
  const currencyIconHtml = data.reward.type === "diamonds"
