...
function initFriendshipCp({
  ...
  findUserByUserId, saveUsers, clampDiamondBalance, logTransaction,
  users, userAuth, io,
  ...
  const FRIENDSHIP_COST = Math.max(1, Number(process.env.FRIENDSHIP_COST_DIAMONDS) || 100000);
  const CP_COST = Math.max(1, Number(process.env.CP_COST_DIAMONDS) || 500000);
  ...
  function addPrivateRelationshipMessage({ fromUserId, toUserId, request, status }) {
    ...
    const text = status === 'pending'
      ? `💞 ${typeName} request from ${findUserByUserId(fromUserId)?.user?.name || 'User'} — ${request.cost.toLocaleString()} Diamonds`
      : status === 'accepted'
  ...
  async function sendRequest(actorId, targetId, type) {
    ...
    if ((sender.user.diamonds || 0) < cost) return { success: false, message: `Not enough Diamonds. ${typeLabel(type)} needs ${cost.toLocaleString()} Diamonds.` };

    sender.user.diamonds = clampDiamondBalance(actorId, sender.user.diamonds - cost, `relationship-${type}-send`);
    logTransaction(actorId, 'diamonds', -cost, `${typeLabel(type)} request sent to ${target.user.name}`);
    saveUsers();
