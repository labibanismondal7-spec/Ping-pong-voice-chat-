...
async function ensureRoom(roomName, { maxParticipants, metadata } = {}) {
    ...
    return client.createRoom({
        ...
        maxParticipants: Number.isFinite(maxParticipants)
            ? maxParticipants
            : Math.max(8, Math.min(500, Number.parseInt(process.env.LIVEKIT_MAX_PARTICIPANTS || "64", 10) || 64)), // 8 seats are publishers, but audience listeners must not consume the final slot; production default supports 8 speakers + listeners.
