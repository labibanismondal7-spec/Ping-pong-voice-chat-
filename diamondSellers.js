...
// this file.
//
// Data stored here (diamond_sellers_list.json) is ONLY the admin-curated list of
...
// seller's userId in the last 30 days. If a listed Diamond Seller has never
// had one (e.g. they aren't also an approved Diamond Seller), the count
// is honestly 0 — never fabricated.

const path = require("path");

function initDiamondSellers(deps) {
    const {
    ...
    const SELLERS_FILE = path.join(DATA_FOLDER, "diamond_sellers_list.json");
    ...
    // --------------------------------------------------
    // PUBLIC — used by the Wallet page's Diamond Seller List.
    // --------------------------------------------------
    app.get("/api/wallet/diamond-sellers", (req, res) => {
    ...
    // --------------------------------------------------
    // ADMIN — Manage Diamond Sellers
    // --------------------------------------------------
    app.get("/api/admin/diamond-sellers", requireAdmin, requirePermission("diamond-seller:view"), (req, res) => {
        const list = Object.values(sellers)
            .slice()
            .sort((a, b) => (a.order || 0) - (b.order || 0))
            .map((record) => {
    ...
    app.post("/api/admin/diamond-sellers/add", requireAdmin, requirePermission("diamond-seller:manage"), (req, res) => {
        const userId = String(req.body.userId || "").trim();
        if (!userId) return res.json({ success: false, message: "Provide a User ID" });
        if (sellers[userId]) return res.json({ success: false, message: "This user is already on the Diamond Seller List" });
        ...
        saveSellers();
        rbac.logAction({
            admin: req.adminAccount, action: "diamond-seller-add", module: "diamond-seller",
            targetType: "diamond-seller", targetId: found.user.userId,
    ...
    app.post("/api/admin/diamond-sellers/:userId/remove", requireAdmin, requirePermission("diamond-seller:manage"), (req, res) => {
        ...
        saveSellers();
        rbac.logAction({
            admin: req.adminAccount, action: "diamond-seller-remove", module: "diamond-seller",
            targetType: "diamond-seller", targetId: req.params.userId,
    ...
    // exactly as shown by GET /api/admin/diamond-sellers.
    app.post("/api/admin/diamond-sellers/reorder", requireAdmin, requirePermission("diamond-seller:manage"), (req, res) => {
        ...
        saveSellers();
        rbac.logAction({
            admin: req.adminAccount, action: "diamond-seller-reorder", module: "diamond-seller",
            targetType: "diamond-seller", targetId: "bulk",
...
module.exports = { initDiamondSellers };
