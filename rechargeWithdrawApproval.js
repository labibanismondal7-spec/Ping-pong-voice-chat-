...
// state machine for admin-recorded manual Recharge (crediting diamonds to a
...
//
// The wallet mutation (diamonds credited / diamonds deducted) only ever
...
function initRechargeWithdrawApproval(deps) {
    const {
        ...
        findUserByUserId, saveUsers, logTransaction, pushWalletUpdate, levelFromDiamondsLegacy,
    ...
    const clampDiamonds = typeof deps.clampDiamondBalance === "function" ? deps.clampDiamondBalance : (userId, diamonds) => {
        ...
        return Math.min(Math.floor(diamonds), 100000000000);
    };
    ...
    // ==================================================
    // RECHARGE — Admin submits an amount of diamonds to credit a user after
    ...
    const recharge = createApprovalWorkflow({
        ...
        validateSubmit(body) {
            ...
            return {
                ...
                data: { domain: "recharge", currency: "diamonds", amount, paymentRef: (body.paymentRef || "").trim() || null }
            };
        },
        onApprove(record) {
            const found = findUserByUserId(record.userId);
            if (!found) return { ok: false, message: "User no longer exists" };
            found.user.diamonds = clampDiamonds(record.userId, (found.user.diamonds || 0) + record.amount, "recharge-approve");
            ...
            saveUsers();
            logTransaction(record.userId, "diamonds", record.amount, "Recharge Approved (" + record.requestId + ")");
