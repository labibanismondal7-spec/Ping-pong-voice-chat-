    ...
    function registerSocketHandlers(socket) {
        ...
        socket.on("call:invite", safeHandler(({ toUserId, callType }) => {
            ...
            const call = {
                ...
                // ICE can be generated before the callee presses Accept. Keep
                // those candidates instead of silently dropping them while
                // calleeSocketId is still null; flush them when the winning
                // device accepts.
                pendingIce: []
            };
        ...
        socket.on("call:accept", safeHandler(({ callId }) => {
            ...
            if (call.pendingIce && call.pendingIce.length) {
                const pending = call.pendingIce.splice(0, call.pendingIce.length);
                pending.forEach(({ fromUserId, candidate }) => {
                    relayTo(call, fromUserId, "call:ice-candidate", { callId, candidate });
                });
            }
        ...
        socket.on("call:ice-candidate", safeHandler(({ callId, candidate }) => {
            ...
            const targetSocketId = socket.userId === call.callerId ? call.calleeSocketId : call.callerSocketId;
            if (!targetSocketId) {
                if (call.pendingIce.length < 200) call.pendingIce.push({ fromUserId: socket.userId, candidate });
                return;
            }
